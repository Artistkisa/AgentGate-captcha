...
AgentGate 返回的 token 是一个判断依据，不是门禁开关。怎么用完全是你的决定：`human_suspected` 答对了也可以拒之门外，特定分数段的 `robot` 也可以放行，`agent` token 上面还可以叠你自己的业务鉴权。AgentGate 给出细粒度的信号（`identity` + `score`），最终裁决权在你的后端。
